...
export default {
  ...
  methods: {
    ...
    cancelAuthUser(row) {
      ...
      this.$modal.confirm('确认要Cancel该User"' + row.userName + '"Role吗？').then(function() {
        return authUserCancel({ userId: row.userId, roleId: roleId });
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess("Cancel授权成功");
      }).catch(() => {});
    },
    ...
    cancelAuthUserAll(row) {
      ...
      this.$modal.confirm('是否Cancel选中User授权Data项？').then(function() {
